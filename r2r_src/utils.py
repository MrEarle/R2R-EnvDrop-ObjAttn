...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        print(f"Using dataset {base_path % split} for split {split} (arg: {args.dataset})")
...
class Tokenizer(object):
    ...
    def __init__(self, vocab=None, encoding_length=20):
        ...
        self.vocab = vocab
        self.word_to_index = defaultdict(lambda: self.word_to_index["<UNK>"])
        self.index_to_word = {}
        if vocab:
    ...
    with open(feature_store, "r") as tsv_in_file:  # Open the tsv file.
        ...
        for item in reader:
            ...
            features[long_id] = np.frombuffer(
                base64.decodestring(item["features"].encode("ascii")), dtype=np.float32
            ).reshape(
                (views, -1)
...
def length2mask(length, size=None):
    ...
    mask = (
        torch.arange(size, dtype=torch.int32).unsqueeze(0).repeat(batch_size, 1)
        > (torch.tensor(length, dtype=torch.int32) - 1).unsqueeze(1)
    ).cuda()
    return mask
