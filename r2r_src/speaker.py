...
class Speaker:
    ...
    def __init__(self, env, listener, tok):
        ...
        # Model
        print("VOCAB_SIZE", self.tok.vocab_size(), flush=True)
    ...
    def load(self, path):
        ...
        print("Load the speaker's state dict from %s" % path, flush=True)
        states = torch.load(path)

        def recover_state(name, model, optimizer):
            # print(name, flush=True)
            # print(list(model.state_dict().keys()), flush=True)
            # for key in list(model.state_dict().keys()):
            #     print(key, model.state_dict()[key].size(), flush=True)
