...
def train(train_env, tok, n_iters, log_every=100, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter + n_iters, log_every):
        ...
        print(
            ("%s (%d %d%%) %s" % (timeSince(start, float(iter) / n_iters), iter, float(iter) / n_iters * 100, loss_str))
        )
                ...
                result_for_eval.append(
                    {
                        ...
                        + max(results[key]["paths"], key=lambda x: cal_score(x, alpha, avg_speaker, avg_listener))[
                            "trajectory"
                        ],
                    }
                )
...
if __name__ == "__main__":
    if args.train in ["speaker", "rlspeaker", "validspeaker", "listener", "validlistener"]:
        train_val()
