...
from obj_attention import ConnectionwiseObjectAttention, ObjectFeatureReducer
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(
        ...
        if bidirectional:
            print("Using Bidir in EncoderLSTM", flush=True)
...
class AttnDecoderLSTM(nn.Module):
    ...
    def __init__(
        ...
        # * === Object Attention ===
        if args.include_objs:
            self.object_feat_reducer = ObjectFeatureReducer(output_dim=obj_attn_size)
            self.connectionwise_obj_attn = ConnectionwiseObjectAttention(obj_attn_size)

            if args.obj_aux_task:
                self.obj_aux_layer = nn.Sequential(
    ...
    def forward(
        ...
        action: batch x angle_feat_size # ! Accion previa?
        ...
        # ! h_tilde tiene la instruccion (creo)
        ...
        if args.include_objs:
            ...
            conn_objs, _ = self.connectionwise_obj_attn(
                ...
                object_feats=object_feats,  # [batch, num_objs, 2048, 2, 2]
...
class SpeakerEncoder(nn.Module):
    def __init__(self, feature_size, hidden_size, dropout_ratio, bidirectional):
        ...
        if bidirectional:
            print("BIDIR in speaker encoder!!", flush=True)
