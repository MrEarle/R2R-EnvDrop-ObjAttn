...
def train_speaker(train_env, tok, n_iters, log_every=500, val_envs={}):
    print("Starting Tensorboard", flush=True)
    writer = SummaryWriter(logdir=log_dir)
    print("Starting Listener (Seq2Seq Agent)", flush=True)
    ...
    for idx in range(start_iter, n_iters, log_every):
        ...
        print(
            f"Starting Speaker Training. Num iters: {n_iters}. Log_every = {log_every}",
            flush=True,
        )
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            print("............ Evaluating %s ............." % env_name, flush=True)
            speaker.env = env
            path2inst, loss, word_accu, sent_accu = speaker.valid()
            path_id = next(iter(path2inst.keys()))
            print("Inference: ", tok.decode_sentence(path2inst[path_id]), flush=True)
            print("GT: ", evaluator.gt[str(path_id)]["instructions"], flush=True)
...
def train(train_env, tok, n_iters, log_every=500, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter + n_iters, log_every):
        ...
        if iter % 50000 == 0:
            listner.save(idx, best_val, os.path.join("snap", args.name, "state_dict", "Iter_%06d" % (iter)))

    listner.save(idx, best_val, os.path.join("snap", args.name, "state_dict", "LAST_iter%d" % (idx)))
...
def beam_valid(train_env, tok, val_envs={}):
    ...
    for env_name, (env, evaluator) in val_envs.items():
        ...
        if args.param_search:
            ...
            print(tmp_result, flush=True)
            # print("Env Name %s" % (env_name), flush=True)
...
def valid_speaker(tok, val_envs):
    ...
    for env_name, (env, evaluator) in val_envs.items():
        ...
        path_id = next(iter(path2inst.keys()))
        print("Inference: ", tok.decode_sentence(path2inst[path_id]), flush=True)
        print("GT: ", evaluator.gt[path_id]["instructions"], flush=True)
        ...
        print("For env %s" % env_name, flush=True)
        print(score_string, flush=True)
        print("Average Length %0.4f" % utils.average_length(path2inst), flush=True)
...
def train_val_augment():
    ...
    print("The training data_size is : %d" % train_env.size(), flush=True)
    print(
        "The average instruction length of the dataset is %0.4f." % (stats["length"]),
        flush=True,
    )
    print(
        "The average action length of the dataset is %0.4f." % (stats["path"]),
        flush=True,
    )
    stats = aug_env.get_statistics()
    print("The augmentation data size is %d" % aug_env.size(), flush=True)
    print(
        "The average instruction length of the dataset is %0.4f." % (stats["length"]),
        flush=True,
    )
